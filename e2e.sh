...
USER_ONE=$(echo $USERS | jq -c '.[] | select(.Username | contains(env.USERNAME_ONE))')

USER_ONE_SUB=$(echo $USER_ONE | jq --raw-output -c '.Attributes[] | select(.Name=="sub") | .Value')

USER_ONE_STATUS=$(echo $USER_ONE | jq --raw-output -c '.UserStatus')

echo "Wallet: '$WALLET_ADDRESS_ONE' Username: '$USERNAME_ONE' Sub: '$USER_ONE_SUB' Status: '$USER_ONE_STATUS'"
